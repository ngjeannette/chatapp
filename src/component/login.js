...
function Login(props) {
    ...
    const axiosCheckLogin = (isgoogleauthenticate, email, password) => {
        if(password) {
            ...
            axios.get('http://localhost:5000/user/checkemail', { params: { email, isgoogleauthenticate, password } })
                .then(res => {
                    // update avatar
                    // update message
            ...
            axios.get('http://localhost:5000/user/checkemail', { params: { email, isgoogleauthenticate } })
                .then(res => {
                    if (res.data.length !== 0) {
                        setMessage('Logged In');
                        props.updateAvatar(res.data[0].username.slice(0, 1).toUpperCase());
    ...
    return (
        <Grommet theme={grommet} >
            <Box direction="column" height={{ min: "calc(100vh - 72px)" }} justify="center" align="center" className="login">
                <Heading margin="none" level={3} margin="small" >Login</Heading>
                {
                    emailLogin ?
                        ...
                        <>
                            <Box width={{ min: "250px" }} margin="small">
                                <Button fill label="Login with Google" onClick={() => { applyGoogleAuthentication() }} icon={<Google color='plain' size='medium' />} />
                            </Box>
                            <Box width={{ min: "250px" }}  >
                                <Button fill gap="13px" label="Login with Email" onClick={({ value }) => { setEmailLogin(true) }} icon={<Mail color='plain' size='medium' />} />
                            </Box>
                        </>
                }
