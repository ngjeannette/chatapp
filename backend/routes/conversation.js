...
router.route('/selectedconvo').get((req,res) => {
    ...
    Conversation.find({ $or: [{ to: req.query.to, from: req.query.from }, { to: req.query.from, from: req.query.to }] })
...
router.route('/addconversation').post((req,res) => {
    const {body: {to, from, chatHistory, read} } = req;
...
router.route('/friends').get((req,res) => {
    ...
    .then(response => {
        ...
        return Promise.all(friends.map(friend => Conversation.find({ to: user, from: friend, read: false }).count()))
            .then((counts) => counts.map((count, i) => ({ friend: friends[i], unread: count })))
    })
